	...
	private class LoadTopTorrents extends AsyncTask<Void, Void, Boolean> {
		...
		protected void onPostExecute(Boolean status) {
			if (status == true) {
				...
				for (int i = 0; i < top.getSections().get(0).getResults().size(); i++) {
					if ((i % 2) == 0) {
					...
					pastDay.get(i)
							.setText(top.getSections().get(0).getResults().get(i).getArtist() + " - " + "Album name [2010]");
				...
				for (int i = 0; i < top.getSections().get(1).getResults().size(); i++) {
					if ((i % 2) == 0) {
					...
					pastWeek.get(i).setText(
							top.getSections().get(1).getResults().get(i).getArtist() + " - " + "Album name [2010]");
				...
				for (int i = 0; i < top.getSections().get(2).getResults().size(); i++) {
					if ((i % 2) == 0) {
					...
					allTime.get(i)
							.setText(top.getSections().get(2).getResults().get(i).getArtist() + " - " + "Album name [2010]");
				...
				for (int i = 0; i < top.getSections().get(3).getResults().size(); i++) {
					if ((i % 2) == 0) {
					...
					snatched.get(i).setText(
							top.getSections().get(3).getResults().get(i).getArtist() + " - " + "Album name [2010]");
				...
				for (int i = 0; i < top.getSections().get(4).getResults().size(); i++) {
					if ((i % 2) == 0) {
					...
					transferred.get(i).setText(
							top.getSections().get(4).getResults().get(i).getArtist() + " - " + "Album name [2010]");
				...
				for (int i = 0; i < top.getSections().get(5).getResults().size(); i++) {
					if ((i % 2) == 0) {
					...
					seeded.get(i).setText(top.getSections().get(5).getResults().get(i).getArtist() + " - " + "Album name [2010]");
					seeded.get(i).setOnClickListener(TopTorrentsActivity.this);
					scrollLayout.addView(seeded.get(i));
				}

			} else {
				Toast.makeText(TopTorrentsActivity.this, "Couldn't not load top 10", Notification.LENGTH_SHORT).show();
			}
			dialog.dismiss();
